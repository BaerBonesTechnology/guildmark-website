...
///
/// Advances an assignment through its workflow states and records evidence
/// (wipe_method, reimage_os, cert_url, tracking_number, carrier).
///
/// The client sends a `{ status, ...evidence }` body. Only the fields
/// provided in the body are updated — omitted fields are left unchanged.
///
/// Valid forward-only transitions:
///   claimed             → wipe_in_progress
///   wipe_in_progress    → wipe_complete
///   wipe_complete       → awaiting_cert            (wipe_only)
///   wipe_complete       → reimage_in_progress      (wipe_and_reimage)
///   reimage_in_progress → reimage_complete
///   reimage_complete    → awaiting_cert
///   awaiting_cert       → cert_uploaded             (requires cert_url)
///   cert_uploaded       → shipped                  (requires tracking_number + carrier)
///   shipped             → complete
///
...
Future<Response> onRequest(RequestContext context, String id) async {
  ...
  if (newStatus == 'shipped') {
    final resolvedTracking = trackingNumber ?? row['tracking_number']?.toString();
    final resolvedCarrier  = carrier;
    if (resolvedTracking == null || resolvedTracking.isEmpty || resolvedCarrier == null) {
      return badRequest(
  ...
  final setClauses   = <String>['status = @status::partner_assignment_status', 'updated_at = now()'];
  ...
  if (wipeMethod     != null) { setClauses.add('wipe_method = @wipe_method');         parameters['wipe_method']      = wipeMethod; }
  if (reimageOs      != null) { setClauses.add('reimage_os = @reimage_os');            parameters['reimage_os']       = reimageOs; }
  if (certUrl        != null) { setClauses.add('cert_url = @cert_url');                parameters['cert_url']         = certUrl; }
  if (trackingNumber != null) { setClauses.add('tracking_number = @tracking_number');  parameters['tracking_number']  = trackingNumber; }
  if (carrier        != null) { setClauses.add('carrier = @carrier');                  parameters['carrier']          = carrier; }
  if (isComplete)             { setClauses.add('completed_at = now()'); }
