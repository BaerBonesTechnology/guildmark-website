...
import { Users as UsersIcon, RefreshCw, Search, X, Trash2 } from "lucide-react";
...
function Badge({ label, styles }: { label: string; styles: string }) {
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-mono border ${styles}`}
    >
      {label}
    </span>
  );
...
export function Users() {
  // Store apiFetch in a ref so it never appears in useCallback deps.
  // useApi() returns a new function reference every render; putting it
  // directly in the dep array would cause load → effect → render → load loops.
  ...
  const load = useCallback(async () => {
    ...
    // Try up to MAX_RETRIES times, stopping immediately on success or
    // after the last failure. No automatic retries after giving up.
  ...
  }, [offset, debouncedSearch, planFilter]); // apiFetch intentionally omitted — see ref above

  // Only fires when filters/offset change — not on error
  useEffect(() => { load(); }, [load]);

  async function deleteUser(id: string) {
    try {
      await apiFetchRef.current(`/admin/users/${id}`, { method: "DELETE" });
              ...
              users.map(u => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    ...
                    {u.size_band && (
                      <p className="text-xs text-slate-600 mt-0.5">
                        {u.size_band} employees
                      </p>
                    )}
                  </td>
                  <td className="px-4 py-3 text-slate-500">
                    {u.industry ?? <span className="text-slate-700">—</span>}
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex flex-col gap-1 items-start">
                      <Badge
                        label={u.plan}
                        styles={PLAN_STYLES[u.plan] ?? PLAN_STYLES.free}
                      />
                      {u.subscription_status !== "active" && (
                        <Badge
                          label={u.subscription_status.replace("_", " ")}
                          styles={STATUS_STYLES[u.subscription_status] ?? ""}
                        />
                      )}
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <Badge
                      label={u.role}
                      styles={ROLE_STYLES[u.role] ?? ROLE_STYLES.member}
                    />
                  </td>
                  <td className="px-4 py-3 text-slate-500">
